import { Order, Transaction, MatchedOrder, MatchingResult } from '../types/domain'
...
function calculateMatchScore(order: Order, transaction: Transaction): number {
  ...
  const customerSimilarity = stringSimilarity(order.customer, transaction.customer)
  return customerSimilarity
}

export function runClientSideMatching(orders: Order[], transactions: Transaction[]): MatchingResult {
...
export function generateSampleData(): { orders: Order[]; transactions: Transaction[] } {
  const orders: Order[] = [
    {
      customer: "John Smith",
      orderId: "ORD001",
